...
public func bech32Polymod(_ values: Data) -> UInt32 {
    var chk: UInt32 = 1
    for v in values {
        let top = (chk >> 25)
...
public func bech32Encode(hrp: String, _ input: [UInt8]) -> String {
    let table: [Character] = Array("qpzry9x8gf2tvdw0s3jn54khce6mua7l")
    let bits = eightToFiveBits(input)
    let check_sum = bech32Checksum(hrp: hrp, data: bits)
    let separator = "1"
...
func bech32ConvertBits(outbits: Int, input: Data, inbits: Int, pad: Int) -> Data? {
...
public func bech32Decode(_ str: String, limit: Bool = true) throws -> (hrp: String, data: Data)? {
    ...
    guard bech32Verify(hrp: hrp, checksum: values) else {
        throw Bech32Error.checksumMismatch
    }
    let out = Data(values[..<(vSize-6)])
    guard let converted = bech32ConvertBits(outbits: 8, input: out, inbits: 5, pad: 0) else {
        return nil
    }
...
public enum Bech32Error: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
            ...
            case .decodeFailed:
                return "Something went wrong decoding"
        }
    }
}
